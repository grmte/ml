...
df = data.frame(tBidGreaterThanAskInNext100=targetVector$V2,fAskP0OfCurrentRow=feature1$V2,fLTPOfCurrentRow=feature2$V2)

print ("Section5: Running logistic regression") 
logistic.fit <- glm (tBidGreaterThanAskInNext100 ~ fAskP0OfCurrentRow+fLTPOfCurrentRow , data = df,family = binomial(link="logit") ) 
