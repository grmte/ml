...
print ("Reading in fRatioOfBidQSumAskQSum.feature") 
feature1=read.csv(paste(args[2],"fRatioOfBidQSumAskQSum.feature",sep=""), header=FALSE) 
print ("Reading in fRatioOfBidAskQ0OfCurrentRow.feature") 
feature2=read.csv(paste(args[2],"fRatioOfBidAskQ0OfCurrentRow.feature",sep=""), header=FALSE) 

print ("Section4: Making sure all feature vectors are of same length") 
if (length(feature1$V1) != length(feature2$V1)) { 
print ("The feature lengths do not match for feature1fRatioOfBidQSumAskQSum and feature2fRatioOfBidAskQ0OfCurrentRow") 
quit() 
}else{ 
...
print ("The feature timestamps do not match for feature1fRatioOfBidQSumAskQSum and feature2fRatioOfBidAskQ0OfCurrentRow") 
quit() 
}else{ 
...
df = data.frame(tBidEqualToAskInNext100=targetVector$V2,fRatioOfBidQSumAskQSum=feature1$V2,fRatioOfBidAskQ0OfCurrentRow=feature2$V2)

print ("Section7: Running logistic regression") 
logistic.fit <- glm (tBidEqualToAskInNext100 ~ fRatioOfBidQSumAskQSum+fRatioOfBidAskQ0OfCurrentRow , data = df,family = binomial(link="logit") ) 

print (paste("Section8: Saving the model in file ./ob/e4/design.model")) 
save(logistic.fit, file = "./ob/e4/design.model")