...
parser.add_argument('-ls',required=True,help='Lot Size in qty')
parser.add_argument('-nTicks',required=True,help='Lot Size in qty')
...
for indexOfCL in range(0,len(gEntryCLList)):
    lInitialFileName = args.a + '-td.' + os.path.basename(os.path.abspath(args.td)) + \
                   ...
                   '-l.'+gEntryCLList[indexOfCL]+"-"+gExitCLList[indexOfCL] + "-" +gEntryCL2List[indexOfCL]+"-"+gExitCL2List[indexOfCL] + "-tq." + args.orderQty + "-te.7WithMultiPredStats"
...
def checkIfPreviousDecisionToEnterOrExitTradeWasSuccessful(pCurrentObj,pPrevObj , pTradeStats):
    ...
    global gOpenSellList, gOpenBuyList, gCloseSellList, gCloseBuyList, lotSize
    ...
    if(pPrevObj.CloseBuy < 0): # Need to buy we come here only if currentPosition is greater than 0 so no need to check again.
        ...
        if lSpread > gTickSize:
            if(l_dummy_TTQChange_For_Buy<=pTradeStats["closeBuyQty"]):
                ...
                gCloseBuyList += [lCloseBuyTradedPrice] * int(lCloseBuyTradedQty / lotSize)
        #hitting
    ...
    if(pPrevObj.OpenBuy > 0 and ( gMaxQty - pTradeStats['currentPositionLong'] ) > 0): # Need to buy
        ...
        if lSpread > gTickSize:
            if(l_dummy_TTQChange_For_Buy <= pTradeStats["openBuyQty"] ):
                ...
                gOpenBuyList += [lOpenBuyTradedPrice] * int(lOpenBuyTradedQty / lotSize)
        #hitting
    ...
    if(pPrevObj.CloseSell < 0): # Need to sell we come here only if currentPosition is greater than 0 so no need to check again.
        ...
        if lSpread > gTickSize:        
            if(l_dummy_TTQChange_For_Sell <= pTradeStats["closeSellQty"] ):
                ...
                gCloseSellList += [lCloseSellTradedPrice] * int(lCloseSellTradedQty / lotSize)
        #hitting
    ...
    if(pPrevObj.OpenSell > 0 and  ( gMaxQty - pTradeStats['currentPositionShort'] ) > 0 ): # Need to sell
        ...
        if lSpread > gTickSize:        
            if(l_dummy_TTQChange_For_Sell <= pTradeStats["openSellQty"]):
                ...
                gOpenSellList += [lOpenSellTradedPrice] * int(lOpenSellTradedQty / lotSize)
        #hitting
...
def update_obj_list(pCurrentDataRow,l_obj):
    ...
    l_obj.WtBidP[0] = float(int(pCurrentDataRow[colNumberOfData.WtBidP0]))
    l_obj.WtBidP[1] = float(int(pCurrentDataRow[colNumberOfData.WtBidP1]))
    if(float(pCurrentDataRow[colNumberOfData.WtAskP0]) % 5 != 0):
        l_obj.WtAskP[0] = float(int(pCurrentDataRow[colNumberOfData.WtAskP0])+ gTickSize)
    if(float(pCurrentDataRow[colNumberOfData.WtBidP1]) % 5 != 0):
        l_obj.WtAskP[1] = float(int(pCurrentDataRow[colNumberOfData.WtBidP1]) + gTickSize)
...
def readOnceAndWrite(pFileName, entryCL , exitCL , entryCL2, exitCL2, predictedValuesList):
    ...
    global gOpenBuyList, gOpenSellList, gCloseSellList, gCloseBuyList, lotSize
    attribute.initList()
    tradeStats = dict()
...
#                pdb.set_trace()
            ...
            if (sum(lLevelDecisionListOS) > 0 and tradeStats['currentPositionLong'] == 0 and tradeStats['currentPositionShort'] < gMaxQty):
                ...
                tradeStats["openSellPrice"] = l_obj.WtAskP[lOpenSell-1] - (OrderMargin*gTickSize)
            ...
            if (sum(lLevelDecisionListCB) > 0 and tradeStats['currentPositionShort'] > 0 and lOpenSell == 0):
                ...
                tradeStats["closeBuyPrice"] = l_obj.WtBidP[-lCloseBuy-1] + (OrderMargin*gTickSize)
            ...
            if (sum(lLevelDecisionListOB) > 0 and tradeStats['currentPositionShort'] == 0 and tradeStats['currentPositionLong'] < gMaxQty):
                ...
                tradeStats["openBuyPrice"] = l_obj.WtBidP[lOpenBuy-1] + (OrderMargin*gTickSize)
            ...
            if (sum(lLevelDecisionListCS) > 0 and tradeStats['currentPositionLong'] > 0 and lOpenBuy == 0):
                ...
                tradeStats["closeSellPrice"] = l_obj.WtAskP[-lCloseSell-1] - (OrderMargin*gTickSize)
...
# Squaring off if some open position there
    if tradeStats['currentPositionLong'] > 0:
        ...
        tradeStats['currentPositionLong'] = 0
        gCloseSellList.append(l_previous_obj.BidP[0])
    ...
    if tradeStats['currentPositionShort'] > 0:
        ...
        tradeStats['currentPositionShort'] = 0
        gCloseBuyList.append(l_previous_obj.AskP[0])
...
#    attribute.writeToFile(fileName , lHeaderColumnNamesList)
    ...
    shortProfitList = [a - b for a,b in zip(gOpenSellList, gCloseBuyList)]
    longProfitList = [a - b for a,b in zip(gCloseSellList, gOpenBuyList)]
    
    lossCount, lossTolCount, lossTolerance, totalProfit, totalLoss = [0,0,50000,0,0]
    for trade in (shortProfitList + longProfitList):
        if trade <= 0:
            lossCount += 1 * lotSize
            totalLoss += trade * lotSize
            if trade * lotSize <= -lossTolerance:
                lossTolCount += 1 * lotSize
        else:
            totalProfit += trade * lotSize
    ...
    print("No of Loss making trades is: " + str(lossCount), file = outputFile)
    print("No of Loss making trades losing more than " + str(lossTolerance) + " is: " + str(lossTolCount), file = outputFile)
    print("Total Loss from loss making trades is: " + str(totalLoss), file = outputFile)
    print("Total Profit from loss making trades is: " + str(totalProfit), file = outputFile)
