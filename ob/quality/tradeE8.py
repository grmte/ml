...
totalEntryCL1 = args.entryCL1.split(";")
totalEntryCL2 = args.entryCL2.split(";")
...
for indexOfCL in range(0,len(totalEntryCL)):
    lInitialFileName = args.a + '-td.' + os.path.basename(os.path.abspath(args.td)) + \
                   ...
                   '-l.'+totalEntryCL[indexOfCL]+"-"+totalEntryCL1[indexOfCL]+"-"+totalEntryCL2[indexOfCL]+"-"+totalExitCL[indexOfCL] + "-tq." + args.orderQty  + "-te.8" 
...
class ticks_values_to_be_stored(object):
    def __init__(self):
        ...
        self.buyPredictValue = 0
        self.sellPredictValue = 0
...
def checkIfPreviousDecisionToEnterOrExitTradeWasSuccessful(pCurrentObj,pPrevObj , pTradeStats):
    ...
    if(pPrevObj.EnterTradeShort == 0 and pPrevObj.EnterTradeLong == 0):
        return [ lReasonForTradingOrNotTradingShort , lReasonForTradingOrNotTradingLong , pPrevObj.BidQ , pPrevObj.AskQ , 0 , 0 ]
    ...
    if(pPrevObj.standingAtBid == 1 and pPrevObj.enterTradeLong == 1 and ( gMaxQty - pTradeStats['currentPositionLong'] ) > 0):
        if(g_bestqty_list_for_buy != None and pPrevObj.BidP <= g_bestqty_list_for_buy['price']):
    ...
    if(pPrevObj.EnterTradeShort == 1 and pPrevObj.standingAtAsk  == 1 and  ( gMaxQty - pTradeStats['currentPositionShort'] ) > 0 ):
        if(g_bestqty_list_for_sell != None and pPrevObj.AskP >= g_bestqty_list_for_sell['price']):
