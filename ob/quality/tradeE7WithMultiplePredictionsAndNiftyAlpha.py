...
for indexOfCL in range(0,len(gEntryCLList)):
    lInitialFileName = args.a + '-td.' + os.path.basename(os.path.abspath(args.td)) + \
                   ...
                   '-l.'+gEntryCLList[indexOfCL]+"-"+gExitCLList[indexOfCL] + "-" +gEntryCL2List[indexOfCL]+"-"+gExitCL2List[indexOfCL] + "-tq." + args.orderQty + "-te.7WithMultiPredWithNifty"
...
def getNiftyDataList(pNiftyFile):
    niftyList = []
    for line in pNiftyFile.readlines()[1:]:
        niftyFeature = float(line.strip().split(";")[1])
        niftyList.append(niftyFeature)
...
def readOnceAndWrite(pFileName, entryCL , exitCL , entryCL2, exitCL2, predictedValuesList, pNiftyLTPList):
    ...
    niftyWALTPChangeTol = 50
    
    for currentDataRow,pObj,niftyLTPChange in zip(dataFile.matrix[10000:],predictedValuesList, pNiftyLTPList[10000:]):
...
#                pdb.set_trace()
            ...
            if(lOpenSell > 0 and lCurrentSellPredictedValue[lOpenSell-1] < lCancelCLOpen[lOpenSell-1]) or tradeStats['currentPositionShort'] == gMaxQty or niftyLTPChange >= gTickSize * niftyWALTPChangeTol:
                lOpenSell = 0
            ...
            if(lOpenBuy > 0 and lCurrentBuyPredictedValue[lOpenBuy-1] < lCancelCLOpen[lOpenBuy-1]) or tradeStats['currentPositionLong'] == gMaxQty or niftyLTPChange <= - gTickSize * niftyWALTPChangeTol:
                lOpenBuy = 0
...
def main():
    ...
    niftyFileName = args.pd.replace('/ro/', '/wf/') + "/f/fColNiftyLTPInCurrentRow[Subtract]fWALTPOfNiftyInLast1Secs-iT.HINDALCO_NIFTY-oT.0-sP.-1.feature"
    niftyFileObj = open(niftyFileName)
    niftyLTPList = getNiftyDataList(niftyFileObj)
    ...
    for lFileName in initialFileName:
        ...
        if os.path.isfile(fileName) and args.skipT.lower() == "yes":
            ...
            continue
        readOnceAndWrite(lFileName,entryCL,exitCL,entryCL2,exitCL2, lListOfAllPredcitionValues, niftyLTPList)
