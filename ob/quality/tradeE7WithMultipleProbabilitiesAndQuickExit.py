...
gCloseSellTolerance = 0
gCloseBuyTolerance = 0
...
for indexOfCL in range(0,len(gEntryCLList)):
    lInitialFileName = args.a + '-td.' + os.path.basename(os.path.abspath(args.td)) + \
                   ...
                   '-l.'+gEntryCLList[indexOfCL]+"-"+gExitCLList[indexOfCL] + "-" +gEntryCL2List[indexOfCL]+"-"+gExitCL2List[indexOfCL] + "-tq." + args.orderQty + "-te.7WithMultiProbQuickExit"
...
def checkIfPreviousDecisionToEnterOrExitTradeWasSuccessful(pCurrentObj,pPrevObj , pTradeStats):
    global gTickSize , gMaxQty , g_quantity_adjustment_list_for_sell , g_quantity_adjustment_list_for_buy, gCloseBuyTolerance, gCloseSellTolerance
...
#    if(pPrevObj.OpenBuy)
    ...
    if(pPrevObj.CloseBuy < 0): # Need to buy we come here only if currentPosition is greater than 0 so no need to check again.
        #standing
        if lSpread > gTickSize and gCloseBuyTolerance == 0:        
...
#     
    ...
    if(pPrevObj.CloseSell < 0): # Need to sell we come here only if currentPosition is greater than 0 so no need to check again.
        #standing
        if lSpread > gTickSize and gCloseSellTolerance == 0:        
...
def readOnceAndWrite(pFileName, entryCL , exitCL , entryCL2, exitCL2, predictedValuesList):
    global transactionCost , currencyDivisor, gMaxQty, gCloseSellTolerance, gCloseBuyTolerance
    attribute.initList()
    tradeStats = dict()
    ...
    lLossTolerance = 10 * gTickSize
    lLastCloseBuyPrice = 0
    lLastCloseSellPrice = 0
...
#                pdb.set_trace()
            ...
            if tradeStats["closeSellPrice"] <> 0:
                lLastCloseSellPrice = tradeStats["closeSellPrice"]
            if tradeStats["closeBuyPrice"] <> 0:
                lLastCloseBuyPrice = tradeStats["closeBuyPrice"]
            ...
            if lLastCloseSellPrice <> 0 and (lLastCloseSellPrice - l_obj.AskP[0]) > lLossTolerance:
                l_obj.CloseSell = -1
                tradeStats["closeSellPrice"] = l_obj.BidP[0]
                tradeStats["closeSellQty"] = 0
                gCloseSellTolerance = 1
            else:
                gCloseSellTolerance = 0
                
            if lLastCloseBuyPrice <> 0 and (l_obj.BidP[0] - lLastCloseBuyPrice) > lLossTolerance:
                l_obj.CloseBuy = -1
                tradeStats["closeBuyPrice"] = l_obj.AskP[0]
                tradeStats["closeBuyQty"] = 0
                gCloseBuyTolerance = 1
            else:
                gCloseBuyTolerance = 0
