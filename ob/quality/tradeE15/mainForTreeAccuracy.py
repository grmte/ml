...
lListOfPredictionDirectory = attribute.getListOfTrainingDirectoriesNames(int(args.nPD),args.pd)
...
for precitionDirectory in lListOfPredictionDirectory:
    ...
    for variable in dd.gTreeVariablesPresent:
        if config['prediction-buy'].get(variable,"f").lower()=="buyprob":
        ...
        elif config['prediction-sell'].get(variable,"f").lower()=="sellprob":
    ...
    if checkAllFilesAreExistOrNot == 'true':
            ...
            dataFileObject = {}
            dataFileObject['buy'] =  open(lWFDirName+"/t/"+config['target']['buy'],"r")
            dataFileObject['sell'] =  open(lWFDirName+"/t/"+config['target']['sell'],"r")
