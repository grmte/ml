...
if 'nsecur' in absPathOfBuyExperimentName:
    pathAfterEBuy = absPathOfBuyExperimentName[absPathOfBuyExperimentName.index("/nsecur/")+8:]
    pathAfterESell = absPathOfSellExperimentName[absPathOfSellExperimentName.index("/nsecur/")+8:]
elif 'nsefut' in absPathOfBuyExperimentName:
    pathAfterEBuy = absPathOfBuyExperimentName[absPathOfBuyExperimentName.index("/nsefut/")+8:]
    pathAfterESell = absPathOfSellExperimentName[absPathOfSellExperimentName.index("/nsefut/")+8:]
elif 'nseopt' in absPathOfBuyExperimentName:
    pathAfterEBuy = absPathOfBuyExperimentName[absPathOfBuyExperimentName.index("/nseopt/")+8:]
    pathAfterESell = absPathOfSellExperimentName[absPathOfSellExperimentName.index("/nseopt/")+8:]
    
if "/" in pathAfterEBuy:
    mainBuyExperimentName = pathAfterEBuy[:pathAfterEBuy.index("/")]
else:
    mainBuyExperimentName = pathAfterEBuy
if "/" in pathAfterESell:
    mainSellExperimentName = pathAfterESell[:pathAfterESell.index("/")]
else:
    mainSellExperimentName = pathAfterESell
...
print(buyExperimentName,mainBuyExperimentName,sellExperimentName,mainSellExperimentName)
...
def getPredictedValuesIntoDict(pPredictedValuesDict):
    ...
    config = ConfigObj(args.eb+"/design.ini")
    target = config["target"]
    lPredictedBuyValuesDict = dict()
    predictedBuyValuesFileName = dirName+"/p/"+mainBuyExperimentName+"/"+args.a + target.keys()[0] + '-td.' + os.path.basename(os.path.abspath(args.td)) + \
                                 '-dt.' + args.dt + '-targetClass.' + args.targetClass + '-f.' + buyExperimentName + "-wt." + args.wt+ attribute.generateExtension() + ".predictions"
    ...
    predictedSellValuesFileName = dirName+"/p/"+mainBuyExperimentName+"/"+args.a + target.keys()[1] + '-td.' + os.path.basename(os.path.abspath(args.td)) + \
                                 '-dt.' + args.dt + '-targetClass.' + args.targetClass + '-f.' + sellExperimentName +  "-wt." + args.wt + attribute.generateExtension()+ ".predictions"
   ...
   for currentDataRow in dataFile.matrix:
       ...
       if currentIndex > 0:
           ...
           listOfStringsToPrint = [ str(bidQ0AtTimeOfPreviousDataRow) , str(bidP0AtTimeOfPreviousDataRow) , str(askP0AtTimeOfPreviousDataRow) , \
                                   ...
                                   str(lDummyTTQForBuy),str(lDummyTTQForSell) , str(currentDataRow[colNumberOfData.ExchangeTS])]
...
# Squaring off if some open position there   
   ...
   attribute.aList[currentIndex-1][1] = tradeStats['currentPositionLong']
   attribute.aList[currentIndex-1][2] = tradeStats['currentPositionShort']
   listOfStringsToPrint = [ str(bidQ0AtTimeOfPreviousDataRow) , str(bidP0AtTimeOfPreviousDataRow) , str(askP0AtTimeOfPreviousDataRow) , str(askQ0AtTimeOfPreviousDataRow) , str(ttqAtTimeOfPreviousDataRow) , str(ltpAtTimeOfPreviousDataRow) , str(currentSellPredictedValue) , str(enterTradeShort) , "" , str(currentBuyPredictedValue) , str(enterTradeLong) ,"" , str(reasonForTrade['CloseBuyTradeHappened']),str(reasonForTrade['OpenBuyTradeHappened']),str(reasonForTrade['OpenSellTradeHappened']),str(reasonForTrade['CloseSellTradeHappened']),str(lDummyBidQ0),str(lDummyAskQ0),str(lDummyTTQForBuy),str(lDummyTTQForSell) , str(currentDataRow[colNumberOfData.ExchangeTS])]
   attribute.aList[currentIndex-1][3] =  ";".join(listOfStringsToPrint) 
   ...
   tradeLogSubDirectoryName =  tradeLogMainDirName + mainBuyExperimentName+"/"
   ...
   lHeaderColumnNamesList  = ['TimeStamp','CurrentPositionLong','CurrentPositionShort','BidQ0','BidP0','AskP0','AskQ0','TTQ','LTP','CurPredValueShort','EnterTradeShort','ReasonForTradingOrNotTradingShort','CurPredValueLong','EnterTradeLong','ReasonForTradingOrNotTradingLong','totalBuyTradeShort','totalBuyLong','totalSellShort','totalSellLong','DummyBidQ0','DummyAskQ0','DummyTTQChangeForSell','DummyTTQChangeForBuy','ExchangeTS']
#    attribute.writeToFile(fileName , lHeaderColumnNamesList)
   ...
   tradeResultSubDirectoryName =  tradeResultMainDirName + mainBuyExperimentName+"/"
...
if __name__ == "__main__":
   ...
   predictedBuyValuesFileName = lWFDirName+"/p/"+mainBuyExperimentName+"/"+args.a + 'buy' + '-td.' + os.path.basename(os.path.abspath(args.td)) + '-dt.' + \
   args.dt + '-targetClass.' + args.targetClass + '-f.' + buyExperimentName + "-wt." + args.wt+ attribute.generateExtension() + ".predictions"
   
   predictedSellValuesFileName = lWFDirName+"/p/"+mainSellExperimentName+"/"+args.a + 'sell' + '-td.' + os.path.basename(os.path.abspath(args.td)) + '-dt.' +\
   ...
   if os.path.isfile(predictedBuyValuesFileName) and os.path.isfile(predictedSellValuesFileName):
       for lFileName in initialFileName:
           fileName = dirName + "/r/" + mainBuyExperimentName + "/" + lFileName+".result"
