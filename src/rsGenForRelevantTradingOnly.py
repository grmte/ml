...
for f in target:
    ...
    for dataDirectory in lListOfTrainingDirectories:
        
        command = aGenForE.genAttribute(attributeName,dataDirectory,args.g,args.tickSize,config)
        utility.runCommand(command, args.run, args.sequence)    

for dataDirectory in lListOfTrainingDirectories:
    command = ['ob/quality/tradeE7ToFindWhichTradesProfitable.py','-orderQty','1','-d',dataDirectory,'-tickSize',args.tickSize,'-e',args.e]
    utility.runCommand(
    command = ['ob/quality/tradeE7OnTargetVariable.py','-orderQty','1','-d','ob/data/ro/nsecur/20140602/','-startTime',"9h00",'-endTime',"17h00",'-tickSize','25000','-targetType','','-e','ob/e/nsecur/live_experiment/']
